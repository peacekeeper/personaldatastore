...
import pds.web.signup.xri.openxri.models.RootNamespacesListModel;
import pds.web.ui.MainWindow;
import echopoint.ImageIcon;

public class Step1ContentPane extends ContentPane {

	private static final long serialVersionUID = 7522573715460619928L;
	...
	private SelectField parentNameSelectField;
	...
	public void init() {
		...
		ListModel listModel = new RootNamespacesListModel(this.openxriXriSignUpMethod.getXriStore());

		this.parentNameSelectField.setModel(listModel);
	}
	...
	private void onRegisterSecondLevelActionPerformed(ActionEvent e) {

		XriStore xriStore = this.openxriXriSignUpMethod.getXriStore();

		String parentName = (String) this.parentNameSelectField.getSelectedItem();
		String localName = this.localNameTextField.getText();
		...
		try {
			...
			Xri parentXri = xriStore.findXri(parentName);

			if (xriStore.existsXri(parentXri, "*" + localName)) {

				this.feedback("Sorry, this I-Name is not available.");
				return;
			}
		} catch (Exception ex) {
		...
		step2ContentPane.setOpenxriXriSignUpMethod(this.openxriXriSignUpMethod);
		step2ContentPane.setParentNameAndLocalName(parentName, localName);
	...
	private void initComponents() {
		...
		label2.setText("Check for availability:");
		column2.add(label2);
		Row row4 = new Row();
		row4.setCellSpacing(new Extent(10, Extent.PX));
		column2.add(row4);
		parentNameSelectField = new SelectField();
		parentNameSelectField.setStyleName("Default");
		parentNameSelectField.setFont(new Font(new Font.Typeface("Arial"),
				Font.BOLD, new Extent(12, Extent.PT)));
		parentNameSelectField.setInsets(new Insets(new Extent(5, Extent.PX)));
		row4.add(parentNameSelectField);
		Label label5 = new Label();
		label5.setStyleName("Default");
		label5.setText("*");
		label5.setFont(new Font(new Font.Typeface("Arial"), Font.BOLD,
				new Extent(18, Extent.PT)));
		row4.add(label5);
		localNameTextField = new TextField();
		localNameTextField.setStyleName("Default");
		localNameTextField.setText("yourname");
		localNameTextField.setInsets(new Insets(new Extent(5, Extent.PX)));
		localNameTextField.addActionListener(new ActionListener() {
			private static final long serialVersionUID = 1L;
	
			public void actionPerformed(ActionEvent e) {
				onRegisterSecondLevelActionPerformed(e);
			}
		});
		row4.add(localNameTextField);
		...
		ResourceImageReference imageReference1 = new ResourceImageReference(
				"/pds/web/signup/xri/openxri/logo-inames.png");
		...
		label3.setText("An I-Name is a single, unified identifier on the Internet that can point to different targets and provide multiple services at the same time. It acts as the primary identifier for your Personal Data Store.");
		column2.add(label3);
		...
		label6.setText("Examples:");
		grid1.add(label6);
		Label label7 = new Label();
		label7.setStyleName("Default");
		label7.setText("=web*andy (I-Name)");
		grid1.add(label7);
		...
		label9.setText("=web*jasmin (I-Name)");
		grid1.add(label9);
		...
		label12.setText("@free*mycompany (I-Name)");
		grid1.add(label12);
		...
		label10.setText("=!238E.19BE.A800.19C2!982E.FF3D.9288.1D24 (I-Number)");
		grid1.add(label10);
	}
}
