...
public class FeedContentPane extends ContentPane implements XdiGraphListener {
	...
	private void addEntry(Date timestamp, String content) throws XdiException {
		...
		feedGraph.createStatement(new XRI3Segment("$" + imageGuid), new XRI3Segment("$d"), Timestamps.dateToXri(timestamp));
		feedGraph.createStatement(new XRI3Segment("$" + imageGuid), new XRI3Segment("$a$xsd$string"), content);
